...
VIEWS.telecom = el => {
  ...
  el.innerHTML = pageHead('Telecom / CDR Intelligence', 'CDRs of account-linked (registered) and alternate numbers, plus investigation numbers. Hits are computed across every uploaded CDR.', `<button id="tlAdd">＋ Investigation number</button><button id="tlImp">⇪ Import CDR</button>`) +
    `<div class="grid g6" style="margin-bottom:12px">${kpi('Target CDRs', tel.targets.length)}${kpi('CDR events', nfmt(c.telecom.cdr.length))}${kpi('Direct hits', tel.direct.length, `${tel.direct.filter(x => x.complainant).length} with complainant`, 'rgba(255,77,94,.18)')}${kpi('Common contacts', tel.common.length)}${kpi('Shared IMEI', tel.sharedImei.length)}${kpi('CDR pending', tel.pendingCdr.length, 'linked numbers without CDR', 'rgba(255,179,0,.18)')}</div>
